  ...
  try {
    ...
    if (parlamentar) {
      ...
      if (rows.length > 0 || transferenciasPix.length > 0) {
        ...
        // Dedup: mesmo numero+ano dentro do contexto do parlamentar.
        // Cobre tanto registros com idPortal idêntico (constraint não aplicada)
        // quanto registros com idPortal diferente mas numero idêntico (mesma emenda
        // importada por caminhos diferentes). rows está ordenado por valorEmpenhado
        // desc, então o primeiro de cada chave é o mais completo.
        const seenKeys = new Set<string>();
        const emendasDedup = emendas.filter((e) => {
          const key = e.numero ? `${e.numero}_${e.ano}` : e.idPortal;
