...
interface ResumoEstado {
  ...
  municipiosLista?: Array<{ codigoIbge: string; nome: string }>;
...
export default function EmendasPage() {
  ...
  useEffect(() => {
    ...
    if (q.length < 2 || !resumo?.municipiosLista) {
      setMunicipioResults([]);
      return;
    }
    const norm = (s: string) => s.normalize('NFD').replace(/[̀-ͯ]/g, '').toLowerCase();
    const qNorm = norm(q);
    setMunicipioResults(resumo.municipiosLista.filter(m => norm(m.nome).includes(qNorm)).slice(0, 15));
  }, [municipioQuery, resumo]);
...
function LegendaCores() {
  ...
  const items = isDark
    ...
    : [
        { label: 'Acima de R$ 2 milhões',   color: '#2C5282' },
        { label: 'R$ 1 milhão – 2 milhões', color: '#4A7FB5' },
        { label: 'R$ 500 mil – 1 milhão',   color: '#82B0D4' },
        { label: 'Até R$ 500 mil',          color: '#B5D0E8' },
        { label: 'Sem emendas',             color: '#E8F0F8', border: '1px solid #CBD5E1' },
      ];
  return (
