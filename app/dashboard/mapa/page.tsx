              ...
              {view === 'municipio' && selectedMunicipio && (
                <div className="h-full flex flex-col">
                  ...
                  <div className="flex-1 flex gap-3 min-h-0">
                    ...
                    {/* Mapa de bairros (pins) — para outros municípios ou SP/RJ/CE no modo zonas */}
                    {(!isSaoPauloCapital || spVisualizacao === 'bairros') && (!isRioDeJaneiro || rjVisualizacao === 'zonas') && (!isFortalezaCe || ceVisualizacao === 'zonas') && (!bairrosLoaded || bairrosData.length > 0) && (
                      <div className="flex-1 min-w-0">
                        <MunicipioMap
                          focusZona={focusZonaReq}
                    ...
                    {/* Lista de zonas — oculta quando SP/RJ/CE estão no modo bairros/distritos */}
                    {!(isSaoPauloCapital && spVisualizacao === 'distritos') && !(isRioDeJaneiro && rjVisualizacao === 'bairros') && !(isFortalezaCe && ceVisualizacao === 'bairros') && <div className={`flex flex-col rounded-xl overflow-hidden ${(bairrosLoaded && bairrosData.length === 0 && !isSaoPauloCapital && !isRioDeJaneiro) ? 'flex-1' : 'w-48'}`}
                      style={{ background: 'rgba(7,29,54,0.8)', border: '1px solid rgba(74,158,222,0.2)' }}>
                      <div className="px-3 py-2 flex items-center gap-2" style={{ borderBottom: '1px solid rgba(74,158,222,0.15)' }}>
                        <Layers className="h-3.5 w-3.5" style={{ color: '#4a9ede' }} />
                        <span className="text-xs font-semibold uppercase tracking-wide" style={{ color: '#4a9ede' }}>Zonas Eleitorais</span>
