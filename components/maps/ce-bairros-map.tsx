...
function CeBairrosMapComponent({ votesData, selectedBairro, onBairroClick, height = '100%' }: CeBairrosMapProps) {
  ...
  useEffect(() => {
    fetch('/geojson/ce-bairros-fortaleza.geojson')
      .then(r => r.json())
  ...
  const getNome = (feature: any): string =>
    feature?.properties?.Nome || feature?.properties?.nome || feature?.properties?.NOME || '';
  ...
  const rebuildVoteLabels = useCallback(async (map: any) => {
    ...
    geoLayer.eachLayer((layer: any) => {
      ...
      try {
        ...
        // MultiPolygon: latlngs[0][0] is the first ring of the first polygon
    ...
    <div className="relative" style={{ height }}>
      ...
      <div className="absolute bottom-3 left-3 z-[1000] bg-white/90 backdrop-blur-md rounded-xl border border-gray-200 px-4 py-2.5 text-xs shadow-md">
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-600 font-medium">Bairros – Fortaleza</span>
        </div>
      </div>
    </div>
  );
...
export default CeBairrosMapComponent;
