...
const EMENDA_STATUS_COLORS: Record<string, string> = {
  PROPOSTA:  '#6366f1', // indigo — proposta
  EMPENHADA: '#f59e0b', // amber — empenhada
  PAGA:      '#10b981', // emerald — paga
  CANCELADA: '#64748b', // slate — cancelada
};
...
function demandaPinSvg(color: string, dotColor: string, foto?: string, selected = false): string {
  ...
  // Pin padrão com inicial da categoria (dotColor = catColor)
...
export default function DemandaMapLeaflet({
  ...
  // Inicializar mapa
  useEffect(() => {
    if (isInitRef.current || !mapRef.current) return;
    isInitRef.current = true;

    (async () => {
      if (isUnmounted()) { isInitRef.current = false; return; }
      ...
      const isMapStale = () => isUnmounted() || mapInstanceRef.current !== map;
      ...
      // ── Camada visual dos distritos de SP (delimitação de bairros)
      if (showSpDistritos) {
        (async () => {
          try {
            ...
            // Mostrar apenas a partir de zoom 11 (cidade inteira)
      ...
      emendas.forEach((em) => {
        ...
        demandBounds.push([em.lat, em.lng]);
      });
      ...
      contatos.forEach((c) => {
        ...
        demandBounds.push([c.lat, c.lng]);
      });

      // Ajustar bounds
      if (demandBounds.length > 0) {
        try {
          map.fitBounds(L.latLngBounds(demandBounds), { padding: [60, 60], maxZoom: 14 });
        } catch (_) {}
      }
  ...
  }, [demands, agendaEvents, emendas, contatos]); // eslint-disable-line react-hooks/exhaustive-deps

  // Centralizar quando o centro externo mudar
  useEffect(() => {
