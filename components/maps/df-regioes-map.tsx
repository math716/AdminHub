...
function DfRegioesMapComponent({ votesData, selectedRegiao, onRegiaoClick, height = '100%' }: DfRegioesMapProps) {
  ...
  useEffect(() => { onRegiaoClickRef.current = onRegiaoClick; }, [onRegiaoClick]);
  useEffect(() => { selectedRegiaoRef.current = selectedRegiao ?? null; }, [selectedRegiao]);
  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      ...
      map.createPane('regioesPane');
      map.createPane('voteLabelsPane');
      const regioesPane = map.getPane('regioesPane');
      const voteLabelsPane = map.getPane('voteLabelsPane');
      if (regioesPane) regioesPane.style.zIndex = '400';
      if (voteLabelsPane) voteLabelsPane.style.zIndex = '450';

      const style = (feature: any, isSelected = false) => ({
        fillColor: isSelected ? '#1565c0' : getColor(feature?.properties?.nome || ''),
      ...
      const geoLayer = L.geoJSON(geoData, {
        ...
        onEachFeature: (feature: any, layer: any) => {
          ...
          layer.on('click', () => {
            ...
            selectedRegiaoRef.current = nome;
            onRegiaoClickRef.current?.(nome);
      ...
      if (selectedRegiaoRef.current) {
        geoLayer.eachLayer((layer: any) => {
          const n = layer.feature?.properties?.nome || '';
          if (normalizeNome(n) === normalizeNome(selectedRegiaoRef.current!)) {
      ...
      // Esconde labels de votos em zoom baixo (DF inteiro visível)
      const updateLabelVisibility = () => {
        const pane = map.getPane('voteLabelsPane');
        if (pane) pane.style.display = map.getZoom() < 10 ? 'none' : '';
      };
  ...
  useEffect(() => {
    ...
    geoLayer.eachLayer((l: any) => {
      ...
      const isSelected = selectedRegiao ? normalizeNome(nome) === normalizeNome(selectedRegiao) : false;
      l.setStyle({
  ...
  }, [votesData, selectedRegiao, getColor, rebuildVoteLabels]);
    ...
    <div className="relative" style={{ height }}>
      ...
      <div className="absolute bottom-3 left-3 z-[1000] bg-white/90 backdrop-blur-md rounded-xl border border-gray-200 px-4 py-2.5 text-xs shadow-md">
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-600 font-medium">Regiões Administrativas – DF</span>
        </div>
      </div>
    </div>
  );
...
export default DfRegioesMapComponent;
