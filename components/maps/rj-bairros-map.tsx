...
// Normalizes removing accents, uppercase, strips non-alphanumeric (handles parentheses in bairro names)
...
function RjBairrosMapComponent({ votesData, selectedBairro, onBairroClick, height = '100%' }: RjBairrosMapProps) {
  ...
  useEffect(() => {
    fetch('/geojson/rj-bairros.geojson')
      .then(r => r.json())
  ...
  const rebuildVoteLabels = useCallback(async (map: any) => {
    ...
    geoLayer.eachLayer((layer: any) => {
      ...
      try {
        ...
        const ring: any[] = latlngs?.[0]?.[0] ?? latlngs?.[0] ?? [];
        const n = ring.length;
        if (n >= 3) {
    ...
    <div className="relative" style={{ height }}>
      ...
      <div className="absolute bottom-3 left-3 z-[1000] bg-white/90 backdrop-blur-md rounded-xl border border-gray-200 px-4 py-2.5 text-xs shadow-md">
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-600 font-medium">Bairros – Rio de Janeiro</span>
        </div>
      </div>
    </div>
  );
...
export default RjBairrosMapComponent;
