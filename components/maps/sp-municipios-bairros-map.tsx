...
interface SpMunicipiosBairrosMapProps {
  municipio: string;
...
function SpMunicipiosBairrosMapComponent({ municipio, votesData, selectedBairro, onBairroClick, height = '100%' }: SpMunicipiosBairrosMapProps) {
  ...
  useEffect(() => {
    ...
    fetch('/geojson/SP_bairros_CD2022.json')
      .then(r => r.json())
      .then((data: any) => {
        const filtered = {
    ...
    geoLayer.eachLayer((layer: any) => {
      ...
      try {
        ...
        const marker = L.marker([centerLat, centerLng], {
          icon: L.divIcon({
            html: `<div style="width:${sz}px;height:${sz}px;background:rgba(7,29,54,0.88);color:#7dd3fc;font-size:${fs}px;font-weight:800;border-radius:50%;border:2px solid rgba(74,158,222,0.5);display:flex;align-items:center;justify-content:center;pointer-events:none;box-shadow:0 2px 6px rgba(0,0,0,0.4);font-family:'Segoe UI',system-ui,sans-serif;letter-spacing:-0.5px;">${label}</div>`,
            className: '',
...
export default SpMunicipiosBairrosMapComponent;
